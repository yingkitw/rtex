...
```bash
git clone https://github.com/yingkitw/latex-rs
cd rtex
cargo build --release
```
...
let html = rtex::convert_tex_string(tex, OutputFormat::Html)?;
```
...
```bash
cargo test --lib example_tests -- --nocapture   # generate example PDFs
