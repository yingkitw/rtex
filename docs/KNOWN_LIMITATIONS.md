...
A **native subset LaTeX converter** — not a replacement for pdflatex/xelatex/lualatex. It parses common LaTeX constructs into an internal AST and renders to PDF/HTML/DOCX/EPUB without shelling out to an external TeX engine. All PDF output is generated by the vendored [pdfrs](https://crates.io/crates/pdfrs) engine.
...
- Multi-line math environments (`align`, `align*`, `gather`, `gather*`, `multline`,
  ...
  `gather`/`multline` render as centered line blocks. Numbering is not yet
  emitted.
