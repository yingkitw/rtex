...
use crate::pdf_generator::{AccessibilityOptions, PageLayout};
use anyhow::Result;
...
impl OptimizedPdfGenerator {
    ...
    /// Accessibility/metadata options (title, tagged PDF, language).
    pub fn with_accessibility(mut self, opts: AccessibilityOptions) -> Self {
        self.accessibility = Some(opts);
        self
    }
